...
class StepwiseEMLearnerExecute(Node):
    def __init__(self):
        super().__init__('stepwise_em_learner_execute')
        ...
        self.declare_parameter('delta_N', 0.1)  # Step size for stepwise EM updates
        ...
        # AIRL parameters (for reward computation on final execution trajectory)
        ...
        self.declare_parameter('use_torch', True)
        ...
        self.delta_N = self.get_parameter('delta_N').value
        ...
        self.result_directory = os.path.join(os.path.expanduser('~/result'), 'stepwise_em_learner_execute')
    ...
    def normalize_demos(self):
        ...
        # Normalize values to [0, 1] range for stepwise EM training
        normalized_scaled = []
    ...
    def train_stepwise_em(self):
        """Train ProMP using stepwise EM learning"""
        if len(self.demos) < 1:
        ...
        try:
            ...
            self.get_logger().info('Training ProMP using stepwise EM learning...')
            ...
            self.get_logger().info(f'Initializing stepwise EM with first demo...')
            self.stepwise_em_learner.initialize_from_first_demo(normalized_demos[0])
            self.get_logger().info(f'Stepwise EM initialized with demo 1/{len(normalized_demos)}')
            
            # Update incrementally with remaining demos
            for i in range(1, len(normalized_demos)):
                self.get_logger().info(f'Updating stepwise EM with demo {i+1}/{len(normalized_demos)}...')
                self.stepwise_em_learner.stepwise_em_update(normalized_demos[i])
                self.get_logger().info(f'Stepwise EM updated with demo {i+1}/{len(normalized_demos)}')
            
            self.get_logger().info('Stepwise EM training completed successfully')
            self.get_logger().info(f'Total demonstrations processed: {self.stepwise_em_learner.N}')
            self.get_logger().info(f'Total time steps: {self.stepwise_em_learner.T_total}')
            self.get_logger().info(f'EM iterations: {self.stepwise_em_learner.em_iterations}')
            
            return True
            
        except Exception as e:
            self.get_logger().error(f'Error training stepwise EM: {e}')
            import traceback
            self.get_logger().error(traceback.format_exc())
            return False
    
    def train_airl(self):
        """Train AIRL on the same demonstrations (same normalization as stepwise EM) for reward evaluation."""
        if len(self.demos) < 1:
            self.get_logger().warn('No demonstrations available for AIRL training')
            return False
        try:
            normalized_demos = self.normalize_demos()
            if len(normalized_demos) == 0:
            ...
            self.get_logger().info('AIRL training completed (for reward evaluation)')
            return True
        except Exception as e:
    ...
    def generate_trajectory(self):
        """Generate trajectory from trained stepwise EM learner and denormalize"""
        if self.stepwise_em_learner is None:
            self.get_logger().error('Stepwise EM learner not trained yet')
            return None
        
        try:
            # Generate trajectory using stepwise EM learner
            trajectory_normalized = self.stepwise_em_learner.generate_updated_trajectory(num_points=self.trajectory_points)
            
            if trajectory_normalized is None:
                self.get_logger().error('Failed to generate trajectory from stepwise EM learner')
                return None
    ...
    def visualize_trajectories(self):
        ...
        try:
            ...
            for i in range(6):
                axes[i].plot(t_learned, self.learned_trajectory[:, i], 'b-', linewidth=2, label='Learned (Stepwise EM)')
    ...
    def _compute_trajectory_reward_airl(self, trajectory_states):
        """
        Compute total reward for a trajectory using the AIRL reward function.
        States are normalized with demo statistics to match AIRL training scale.
        """
        ...
        for s, a in zip(states_norm, actions):
            total += self.airl.compute_reward(s, a)
        return total
    
    def save_final_execution_reward_to_csv(self):
        """
        Compute reward for the final execution trajectory (generated by this script)
        using the AIRL reward function, and save to a CSV file.
        """
        try:
            if len(self.demos) == 0:
                self.get_logger().debug('No demos; skipping final execution reward CSV')
                return False
            if self.demo_min is None or self.demo_max is None:
                self.normalize_demos()
            if self.airl is None and not self.train_airl():
                self.get_logger().warn('AIRL not trained; skipping final execution reward CSV')
                return False
            # Final execution trajectory = what was executed, or the generated trajectory
            if len(self.execution_trajectory_log) > 0:
                trajectory = self.execution_trajectory_log
            elif self.learned_trajectory is not None:
                trajectory = self.learned_trajectory
            else:
                self.get_logger().warn('No trajectory to evaluate; skipping reward CSV')
                return False
            reward_val = self._compute_trajectory_reward_airl(trajectory)
            if reward_val is None:
                self.get_logger().warn('Could not compute reward for final execution trajectory')
                return False
            os.makedirs(self.result_directory, exist_ok=True)
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            rewards_file = os.path.join(self.result_directory, f'final_execution_reward_{timestamp}.csv')
            with open(rewards_file, 'w', newline='') as f:
                writer = csv.writer(f)
                writer.writerow(['reward_type', 'final_execution'])
                writer.writerow(['total', reward_val])
            self.get_logger().info(f'Saved final execution reward to {rewards_file} (total={reward_val:.4f})')
            return True
        except Exception as e:
            self.get_logger().error(f'Error saving final execution reward to CSV: {e}')
            import traceback
            self.get_logger().error(traceback.format_exc())
            return False
    ...
    def save_execution_data_to_csv(self):
        ...
        try:
            ...
            # Compute and save final execution trajectory reward (AIRL) to CSV
            self.save_final_execution_reward_to_csv()
    ...
    def save_learned_trajectory(self, filename=None):
        ...
        try:
            ...
            if filename is None:
                cartesian_file = os.path.join(self.trajectory_save_dir, 'learned_trajectory_stepwise_em.npy')
            else:
            ...
            if self.learned_joint_trajectory is not None:
                joint_file = os.path.join(self.trajectory_save_dir, 'learned_joint_trajectory_stepwise_em.npy')
    ...
    def run_complete_pipeline(self):
        """Run complete pipeline: train stepwise EM and execute"""
        self.get_logger().info('Starting complete Stepwise EM ProMP pipeline...')
        
        # Step 1: Train stepwise EM
        if not self.train_stepwise_em():
            self.get_logger().error('Stepwise EM training failed')
            return False
        
        # Step 1b: Train AIRL on same demos for reward evaluation of final execution
        self.train_airl()
...
def main(args=None):
    ...
    parser = argparse.ArgumentParser(description='Stepwise EM Learner Execute')
    ...
    parser.add_argument('--delta-N', type=float, default=0.1, help='Step size for stepwise EM updates')
    ...
    try:
        ...
        if success:
            node.get_logger().info('Stepwise EM pipeline completed successfully')
        else:
            node.get_logger().error('Stepwise EM pipeline failed')
