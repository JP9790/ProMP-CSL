...
class ProMPConditionController(Node):
    ...
    def train_airl(self):
        """Train AIRL on the same demonstrations (same normalization as ProMP) for reward evaluation."""
        if len(self.demos) < 1:
    ...
    def train_and_generate_trajectory(self):
        ...
        # Step 2b: Train AIRL on same demos for reward evaluation of final execution
        self.train_airl()
    ...
    def save_final_execution_reward_to_csv(self):
        """
        Compute reward for the final execution trajectory (generated/conditioned by this controller)
        ...
        try:
            ...
            if self.airl is None and not self.train_airl():
                ...
                return False
            # Final execution trajectory = what was executed, or the current trajectory
            if len(self.execution_trajectory_log) > 0:
                trajectory = self.execution_trajectory_log
            elif self.current_trajectory is not None:
                trajectory = self.current_trajectory.tolist() if isinstance(self.current_trajectory, np.ndarray) else list(self.current_trajectory)
            else:
